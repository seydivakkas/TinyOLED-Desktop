...
export class Desktop {
  ...
  _registerApps() {
    ...
    if (this._launcher.appCount !== 57) {
      console.error(`Launcher parity error: expected 57 apps, got ${this._launcher.appCount}`);
    }
  }
  ...
  run() {
    this._input.start();
    this._notif.push('57 app simulator hazir!', 2.0);
    this._emitState();
  ...
  _render() {
    const fb = this._fb;
    fb.clear();
    if (this._state === State.SPLASH) this._drawSplash();
    else if (this._state === State.HOME) { this._statusbar.draw(fb); this._launcher.draw(fb); }
    else if (this._state === State.APP && this._app) { this._statusbar.draw(fb); this._app.draw(fb); }
    if (this._notif.active) this._notif.draw(fb);
    this._display.render(fb);
    if (this._state === State.SPLASH) {
      this._splashT++;
      if (this._splashT >= SPLASH_FRAMES) { this._state = State.HOME; this._emitState(); }
    }
  }
  ...
  _drawSplash() {
    ...
    if (t < 15) fb.circle(64, 32, t * 2, true, true);
    else {
      fb.circle(64, 32, 30, true, true);
      fb.textCentered('TinyOLED', 20, false);
      fb.textCentered('57 App Web', 30, false);
